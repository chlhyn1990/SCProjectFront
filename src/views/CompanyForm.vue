<template>
  <div>
    <h4>
      ...
      <a class="btn btn-primary main-btn" data-toggle="modal" data-target="#myModal2" @click="openModal(0)">회사 추가</a>
    </h4>
    <form class="search-from" @submit.prevent="handleSubmit">
      <input type="search" v-model="search" placeholder="회사명"/>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
              ...
              <div class="input-group">
                ...
                <select class="form-select form-select-lg" id="company_grade_idx" v-model="selectedGrade">
              ...
              <div class="form-group">
              <div class="input-group">
                <label class="labelModal" key="" for="company_name">회사이름</label>
                <input 
                  type="text" 
                  id="company_name" 
                  class="form-control"
...
export default {
  ...
  setup() {
    ...
    const openModal = (row) => {
      ...
      selectedGrade.value = row.company_grade_idx; // 선택한 행 데이터 복사
      selectedCompanyName.value = row.company_name; // 선택한 행 데이터 복사
    ...
    const handleSubmit = () => {
      ...
      axios.post(getCompanyList+'/'+search.value,options)
      .then(response=>{ 
    ...
    const saveData = () => {
      ...
      if(selectedIdx.value != '' && selectedIdx.value != undefined)
        reqUrl = updateCompany;
      else
        reqUrl = insertCompany;
      ...
      axios.post(reqUrl, {
            ...
            company_grade_idx: selectedGrade.value,
    ...
    const deleteData = () => {
      ...
      axios.post(deleteCompany+selectedIdx.value, options)
      .then(response=>{ 
        console.log(response.data); // 서버에서 받아온 데이터를 테이블에 반영
        alert('삭제되었습니다');
        fetchData();
