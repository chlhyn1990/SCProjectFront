<template>
  <div>
    <h4>
      ...
      <a class="btn btn-primary main-btn" data-toggle="modal" data-target="#myModal2" @click="openModal(0)">매니저 추가</a>
    </h4>
    <form class="search-from" @submit.prevent="handleSubmit">
      <input type="search" v-model="search" placeholder="직원명"/>
        ...
        <div class="modal-content">
          <div class="modal-body">
            <div class="form-group">
              ...
              <div class="form-group">
              <div class="input-group">
                <label class="labelModal" key="" for="manager_grade_idx">직원등급</label>
                <select class="form-select form-select-lg" id="manager_grade_idx" v-model="selectedGrade">
                  <option :value=row.idx v-for="(row, index) in tableDataGrade" :key="index">{{row.grade_name}}</option>
                </select>
              </div>
            </div>
              <div class="form-group">
              <div class="input-group">
                <label class="labelModal" key="" for="id">아이디</label>
                <input 
                  type="text" 
                  id="id" 
                  class="form-control"
            ...
            <div class="form-group">
              <div class="input-group">
                <label class="labelModal" key="" for="name">이름</label>
                <input 
                  type="text" 
                  id="name" 
                  class="form-control"
            ...
            <div class="form-group">
              <div class="input-group">
                <label class="labelModal" key="" for="phone">연락처</label>
                <input 
                  type="text" 
                  id="phone" 
                  class="form-control"
...
export default {
  ...
  setup() {
    ...
    const selectedId = ref(); // 선택된 행 데이터
    const selectedPassword = ref(); // 선택된 행 데이터
    const selectedName = ref(); // 선택된 행 데이터
    const selectedPhone = ref(); // 선택된 행 데이터
    ...
    const handleSubmit = () => {
      ...
      axios.post(getManagerList+'/'+search.value,options)
      .then(response=>{ 
    ...
    const fetchDataGrade = () => {
      axios.post(getManagerGradeList)
      .then(response=>{ 
        tableDataGrade.value = response.data; // 서버에서 받아온 데이터를 테이블에 반영
        console.log(tableDataGrade);
      })
      .catch(response=>{
        console.error("데이터 요청 실패:", response.status);
      });
    ...
    const saveData = () => {
      ...
      if(selectedIdx.value != '' && selectedIdx.value != undefined)
        reqUrl = updateManager;
      else
        reqUrl = insertManager;
    ...
    const deleteData = () => {
      ...
      axios.post(deleteManager+selectedIdx.value, options)
      .then(response=>{ 
        console.log(response.data); // 서버에서 받아온 데이터를 테이블에 반영
        alert('삭제되었습니다');
        fetchData();
